...
if (isset($_POST['updateInfo'])) {
    ...
    if ($fname !== $currentData['first_name'] ||
        ...
        $stmt = $conn->prepare("UPDATE coordinators_account SET first_name=?, middle_name=?, last_name=?, complete_address=?, phone_number=? WHERE id=?");
        $stmt->execute([$fname, $mname, $lname, $c_address, $cp_number, $coordinatorID]);
        ...
        if ($stmt->rowCount() > 0) {
          ...
          $sql2->execute([$coordinatorID, $logs, $date, $time]);
...
if (isset($_POST['updatePword'])) {
    ...
    $password = $currentData['coordinators_password'];

    if ($pword == $password) {
        if ($npword == $rnpword) {

            // Prepare and execute the SQL update query
        $stmt = $conn->prepare("UPDATE coordinators_account SET coordinators_password = ? WHERE id=?");
        $stmt->execute([$npword, $coordinatorID]);
...
<head>
    ...
    <title>OJT Web Portal: Coordinators Settings</title>
    <!-- ================= Favicon ================== -->
    <!-- Standard -->
...
<form action="" method="POST" class="row g-3 needs-validation" novalidate="">
  <div class="col-md-4 position-relative">
    ...
    <input type="hidden"name="coordinatorID"value="<?php echo $data['id']; ?>" required="">
