...
if (isset($_POST['updateInfo'])) {
    ...
    if ($fname !== $currentData['first_name'] ||
        ...
        $c_address !== $currentData['address'] ||
        $cp_number !== $currentData['phone_number']) {

        // Prepare and execute the SQL update query
        $stmt = $conn->prepare("UPDATE admin_account SET first_name=?, middle_name=?, last_name=?, address=?, phone_number=? WHERE id=?");
        ...
        if ($stmt->rowCount() > 0) {
          ...
          $sql2 = $conn->prepare("INSERT INTO admin_system_notification(admin_id, logs, logs_date, logs_time) VALUES (?, ?, ?, ?)");
...
if(isset($_SESSION['auth_user']['admin_id'])){
  ...
  $studID = $_SESSION['auth_user']['admin_id'];
...
<form action="" method="POST" class="row g-3 needs-validation" novalidate="">
  ...
  <div class="col-md-4 position-relative">
    ...
    <input type="text" class="form-control" name="address" id="validationTooltip02" value="<?php echo $data['address']; ?>" required="">
    <div class="valid-tooltip">
      Looks good!
    </div>
    <div class="invalid-tooltip">
